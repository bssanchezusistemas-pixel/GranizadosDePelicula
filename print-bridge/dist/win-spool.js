...
export async function isWindowsPrinterAvailable(printerName) {
    ...
    try {
        await execFileAsync("powershell.exe", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", cmd], { timeout: 8000, windowsHide: true });
        return true;
    }
    catch {
        return false;
...
export async function printRawWinSpool(printerName, data) {
    const tmp = path.join(os.tmpdir(), `granizados-ticket-${Date.now()}-${Math.random().toString(16).slice(2)}.bin`);
    ...
    try {
        ...
        const detail = err instanceof Error && "stderr" in err
            ...
            : err instanceof Error
                ? err.message
                : String(err);
        throw new Error(`Spooler Windows no pudo imprimir en "${printerName}". ${detail.trim()}`);
    }
    finally {
