...
import { getEffectivePrintMode, getPrintMode, } from "./printer.js";
import { getTcpHost, getTcpPort, isTcpPrinterReachable, printRawTcp, } from "./tcp-print.js";
import { isWindowsPrinterAvailable, printRawWinSpool } from "./win-spool.js";
const execFileAsync = promisify(execFile);
export function resolveRawPrintTarget() {
...
export async function detectPrinterHostFromWindows(printerName) {
    ...
    try {
        const { stdout } = await execFileAsync("powershell.exe", ["-NoProfile", "-ExecutionPolicy", "Bypass", "-Command", cmd], { timeout: 10000, windowsHide: true });
...
export async function isRawPrinterReady() {
    const mode = getEffectivePrintMode();
    if (mode === "tcp") {
        try {
...
export async function sendRawBytes(data) {
    ...
    if (mode === "tcp") {
        console.log(`[print-bridge] Enviando ${data.length} bytes por RED → ${target.label}`);
    ...
    console.log(`[print-bridge] Enviando ${data.length} bytes por Windows → "${name}"`);
    await printRawWinSpool(name, data);
}
/** Si PRINTER_MODE=auto, elige tcp si hay IP (env o Windows) o winspool. */
export async function resolveAutoPrintMode() {
    if (process.env.PRINTER_HOST?.trim())
        return "tcp";
    const name = process.env.PRINTER_NAME?.trim();
...
export function getConfiguredPrintMode() {
    return getPrintMode();
}
