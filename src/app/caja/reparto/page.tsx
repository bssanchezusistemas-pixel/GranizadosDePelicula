"use client";

import { RepartoBoard } from "@/components/caja/RepartoBoard";
import { useVentas } from "@/context/VentasContext";

export default function RepartoPage() {
  const { pedidos, bases, toggleBase, aceptarPedido } = useVentas();

  return (
    <main className="mx-auto max-w-7xl px-4 py-6 sm:px-6">
      <div className="mb-6">
        <p className="text-[11px] font-bold uppercase tracking-[0.3em] text-neon">
          Tablero de reparto
        </p>
        <h1 className="font-[family-name:var(--font-display)] text-2xl uppercase text-white sm:text-3xl">
          Domicilios en ruta
        </h1>
        <p className="mt-1.5 text-sm text-white/40">
          Los repartidores ven aquí los pedidos asignados y los aceptan para
          salir.
        </p>
      </div>

      <RepartoBoard
        pedidos={pedidos}
        bases={bases}
        onToggleBase={toggleBase}
        onAceptar={aceptarPedido}
      />
    </main>
  );
}
